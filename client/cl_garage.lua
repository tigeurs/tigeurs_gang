function openGarageMenu(garageGang)
    ...
    menuGarage:SetRectangleBanner(garageGang.Couleurs.R, garageGang.Couleurs.G, garageGang.Couleurs.B, 200)
                ...
                if v.stock == 0 then
                    ...
                    RageUI.Button((" ~r~> ~s~%s"):format(v.name), nil, { RightLabel = ("[~r~%s~s~]"):format(tostring(v.stock)) }, true, {
                        onSelected = function()
                            ...
                            SetVehicleWindowTint(Vehicule, 2)
                            SetVehicleWindowTint(Vehicule, 3)
